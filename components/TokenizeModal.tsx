...
const TokenizeModal: React.FC<TokenizeModalProps> = ({
  ...
  const [tokenName, setTokenName] = useState(title || (contentType === 'audio' ? 'My Audio Token' : 'My Video Token'));
  ...
  const [category, setCategory] = useState(contentType === 'audio' ? 'Audio' : 'Video');
      ...
      <div className="tokenize-modal">
        ...
        <div className="modal-body">
          ...
          <div className="document-info-box">
            <h3>{contentType === 'audio' ? '📻' : '🎬'} {title || (contentType === 'audio' ? 'Untitled Audio' : 'Untitled Video')}</h3>
            <p>{Math.floor(duration / 60)}:{(duration % 60).toString().padStart(2, '0')} • {(size / (1024 * 1024)).toFixed(1)}MB • Ready for tokenization</p>
          </div>
